...
const db = mysql.createConnection({
  ...
  password: process.env.DB_PASSWORD || "MySQLnirbhay001",
...
app.post("/watchlist", (req, res) => {
  ...
  const sql =
    "INSERT INTO watchlist(user_id,tmdb_film_id,film_title,poster_url) VALUES (?,?,?,?)";
  db.query(
    sql,
    [user_id, tmdb_film_id, film_title, poster_url],
    (err, result) => {
      if (err)
...
app.post("/log", (req, res) => {
  const { user_id, tmdb_film_id, film_title, poster_url, rating, mood } =
    req.body;
  const sql =
    "INSERT INTO logs (user_id,tmdb_film_id,film_title,poster_url,rating,mood) VALUES (?,?,?,?,?,?)";
  db.query(
    sql,
    [user_id, tmdb_film_id, film_title, poster_url, rating, mood],
    (err, result) => {
...
app.get("/following/:userId", (req, res) => {
  ...
  db.query(sql, [req.params.userId], (err, results) => {
    if (err)
      return res.status(500).json({ error: "Could not get following list" });
    res.json(results);
  });
});
